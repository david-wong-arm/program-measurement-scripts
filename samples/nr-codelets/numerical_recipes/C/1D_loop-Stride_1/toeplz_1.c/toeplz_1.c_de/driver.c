...
int main (int argc, char** argv)
{
	...
	unsigned long long alignedSize_r = alignUp(sizeof(double)*arg_n*2, align);
	char* buf_r = malloc (alignedSize);
	double *arg_r = (double*) buf_r;

	unsigned long long alignedSize_x = alignUp(sizeof(double)*arg_n, align);
	char* buf_x = malloc (alignedSize);
	double *arg_x = (double*) buf_x;



	if (!arg_g || !arg_r || !arg_x)
	{
	...
	memset (arg_r, 0, 2*arg_n * sizeof(double));
	memset (arg_x, 0, arg_n * sizeof(double));
        double arg_res[2];
	memset (arg_res, 0, 2 * sizeof(double));
...
//	rdtscll (before);
	...
	{
		nb_iters += codelet_ (arg_n, arg_m, arg_r, arg_x, arg_g, arg_m1, arg_res);
	}
        measure_stop_();
