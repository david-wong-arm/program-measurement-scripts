...
int codelet_ (int n, double dat[2*n+2]);
...
int main (int argc, char** argv)
{
	...
	unsigned long long alignedSize = alignUp(sizeof(double)*(2*nb_elements+2), align);
	char* buf = malloc (alignedSize);
	n = nb_elements;
	double *dat = (double*) buf;
	...
	memset (dat, 0, (2*n+2) * sizeof(double));
