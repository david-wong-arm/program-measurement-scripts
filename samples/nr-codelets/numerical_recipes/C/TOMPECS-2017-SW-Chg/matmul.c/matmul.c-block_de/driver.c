...
int read_arguments (int* nb_elements,int* repetitions, int* bsize)
{
	FILE* file;
	...
	if (file != NULL)
	{
		...
		res = fscanf (file, "%d %d %d", repetitions, nb_elements, bsize);
		if (res != 3)
		{
			fclose (file);
			return -1;
...
int main (int argc, char** argv)
{
	...
	int nb_elements, repetitions, bsize;
	...
	if (read_arguments (&nb_elements, &repetitions, &bsize) == -1){
...
//	rdtscll (before);
	...
	{
		nb_iters += codelet_ (arg_n, arg_a, arg_b, arg_c, bsize);
	}
        measure_stop_();
