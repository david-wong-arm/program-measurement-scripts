...
extern void kernel_target (uint64_t nrep, uint64_t niter, void *s1, void *s2);
extern void kernel_warmup (uint64_t nrep, uint64_t niter, void *s1, void *s2);
...
int main (int argc, char *argv[]) {
   ...
   for (m=0; m<nmet; m++) {
      ...
      if (m == 0) {
         ...
         clock_t start = clock();
         kernel_warmup (nrepw, niter, s1,s2);
         clock_t stop = clock();
         ...
         kernel_warmup (1, niter, s1,s2);
      }

      // measure
      measure_init_();
      measure_start_();
      kernel_target (nrepm, niter, s1,s2);
      measure_stop_();
